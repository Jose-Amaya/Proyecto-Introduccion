# Receives the downloaded data, parses it and returns an array with a boolean and a list of objects, if it fails, the boolean is False
import os
...
def parseData():
    ...
    # Check if there's an error getting the data and return it if so
    errorDN = dataDN.find("ERROR DATAGET") != -1;
    errorOI = dataOI.find("ERROR DATAGET") != -1;

    if errorDN or errorOI:
        toReturn = [None, None] # None means there was no error with the particular piece of data
        if errorDN:
            toReturn[0] = dataDN
        if errorOI:
            toReturn[1] = dataOI
    ...
    for i in range(0, len(Centrales)):
        ...
        if len(Centrales[i].DNlist) is not 0:
            ...
            for cName in Centrales[i].DNlist[1:]: # Put all the possible options
                pattern += "|" + cName
            ...
            if len(data) is 0:
                ...
                Centrales[i].DNgen = listAdd(list(map(lambda x: ast.literal_eval(x), data))) # The data is csv, we can eval it, the result is a tuple but we need a list to then add every found list
                Centrales[i].DNfound = True
    
        # Oferta inicial # Same as above, but with a different regex pattern (not into a function because the data is saved in different variables, also it's only twice)
        
        if len(Centrales[i].OIlist) is not 0:
            toRegex = lambda s: '(?:%s) , D, *(.*)' % s 
            pattern = Centrales[i].OIlist[0]
        
            for cName in Centrales[i].OIlist[1:]:
                pattern += "|" + cName
